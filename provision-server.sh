...
ssh -t -t root@104.236.192.176 <<'EOF'
  nvm install 6
  npm i -g pm2
  exit
EOF
...
##    ssh-keyscan -H -p 22 104.236.192.176 >> ~/.ssh/known_hosts
##    ssh -A -t -t root@104.236.192.176 'bash -s' < deploy.sh
